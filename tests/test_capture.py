"""End to end, against the real ASGI app."""
...
def client(monkeypatch):
    ...
    # reimport so the frozen Settings snapshot picks up the env vars
    import importlib
...
def test_decoy_surface_does_not_advertise_the_framework(client):
    ...
    # docs endpoints off: a honeypot announcing "FastAPI" isn't imitating anything
...
def test_verdict_is_never_leaked_to_the_client(client):
    ...
    assert not leaked, f"profiling has to stay invisible to the client: {leaked}"
...
def test_login_post_is_recorded_without_the_password(client):
    ...
    # the API doesn't expose the body, and the stored copy is already redacted
    assert "Sup3rS3cret!" not in str(events[0])


def test_login_always_fails(client):
    # never grant access. a popped honeypot is someone's foothold
    for _ in range(3):
...
def test_velocity_escalates_verdict_across_requests(client):
    # one /admin hit is a probe, forty distinct paths is a scan
    for i in range(40):
